...
df_all <- bind_rows(
  load_dimension_data("arousal"),
  load_dimension_data("valence")
) %>%
  mutate(
...
p_interaction <- ggplot(df_all, aes(x = dimension, y = Mean_RMSE, color = model)) +
  ...
  theme_minimal() +
  labs(title = "Interacción Modelo × Dimensión", y = "RMSE")
...
p_spaghetti <- ggplot(df_all,
                      aes(x = dimension, y = Mean_RMSE, group = Subject)) +
  geom_line(alpha = 0.4, color = "gray50") +
  geom_point(aes(color = dimension), size = 2) +
  theme_minimal() +
  labs(title = "Variación inter-sujeto en el efecto de dimensión",
       y = "RMSE", x = "Dimensión")
