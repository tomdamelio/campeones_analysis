#!/usr/bin/env python
"""Alpha vs beta scatter — Sanity check 3.4.

Cada punto = una época (pre o post). Ejes: potencia alfa (x) vs beta (y).
Pre (azul) vs post (rojo) en el espacio alfa × beta.

ROIs: occipital (O1/O2) y temporal (T7/T8).

Grid 1×2: occipital | temporal

Usage
-----
    micromamba run -n campeones python scripts/validation/38d_alpha_beta_scatter.py --subject 27
"""
...
def load_pre_post_data(subject: str):
    pre_list, post_list = [], []
    sfreq_out, ch_names_out = None, None
    ...
    return (np.concatenate(pre_list, axis=0),
            np.concatenate(post_list, axis=0),
            sfreq_out, ch_names_out)


def band_power_per_epoch(data: np.ndarray, sfreq: float,
                         flo: float, fhi: float) -> np.ndarray:
    """data: (n, ch, t) → (n,) mean power over channels."""
    n_ep, n_ch, n_t = data.shape
    out = np.empty((n_ep, n_ch))
    for i in range(n_ep):
        for c in range(n_ch):
            freqs, psd = scipy_welch(data[i, c], fs=sfreq, nperseg=n_t)
            mask = (freqs >= flo) & (freqs <= fhi)
            out[i, c] = np.trapz(psd[mask], freqs[mask]) if mask.any() else 0.0
    return out.mean(axis=1)


def plot_scatter(ax, pre_alpha, pre_beta, post_alpha, post_beta, roi_name):
    ax.scatter(pre_alpha,  pre_beta,  c="steelblue", alpha=0.55, s=25,
               label=f"pre  (n={len(pre_alpha)})", zorder=2)
    ax.scatter(post_alpha, post_beta, c="tomato",    alpha=0.55, s=25,
               label=f"post (n={len(post_alpha)})", zorder=2)

    # Means as large markers
    ax.scatter([pre_alpha.mean()],  [pre_beta.mean()],
               c="steelblue", s=120, marker="D", edgecolors="k", lw=1.2, zorder=3)
    ax.scatter([post_alpha.mean()], [post_beta.mean()],
               c="tomato",    s=120, marker="D", edgecolors="k", lw=1.2, zorder=3)

    ax.set_xlabel("Alpha power (µV²)", fontsize=9)
    ax.set_ylabel("Beta power (µV²)", fontsize=9)
...
def main() -> None:
    ...
    fig, axes = plt.subplots(1, len(roi_list), figsize=(12, 5))
    fig.suptitle(
        f"sub-{sub} — Alpha vs Beta scatter: pre (azul) vs post (rojo)\n"
        "Sanity check 3.4  |  Diamante = media de cada clase",
        fontsize=11,
    )

    for ax, (roi_name, roi_chs) in zip(axes, roi_list):
        avail = [ch for ch in roi_chs if ch in ch_names]
        if not avail:
            ax.set_visible(False)
            continue
        ch_idx = [ch_names.index(ch) for ch in avail]
        pre_roi  = X_pre[:, ch_idx, :]
        post_roi = X_post[:, ch_idx, :]
        pre_alpha  = band_power_per_epoch(pre_roi,  sfreq, *ALPHA_BAND)
        pre_beta   = band_power_per_epoch(pre_roi,  sfreq, *BETA_BAND)
        post_alpha = band_power_per_epoch(post_roi, sfreq, *ALPHA_BAND)
        post_beta  = band_power_per_epoch(post_roi, sfreq, *BETA_BAND)
        plot_scatter(ax, pre_alpha, pre_beta, post_alpha, post_beta, roi_name)

    plt.tight_layout()
    out_path = out_dir / f"sub-{sub}_38d_alpha_beta_scatter.png"
