...
# Decoding-specific epoch parameters
TMIN = -2.5
TMAX = 2.0
BASELINE = (-2.5, -1.5)
...
def _select_best_c(X_train, y_train, use_pca: bool) -> float:
...
def run_pipeline(subject: str) -> None:
    print("=" * 60)
    print(f"27 — Photo decoding — sub-{subject}")
    print(f"     TMIN={TMIN}, TMAX={TMAX}, BASELINE={BASELINE}")
    ...
    # Save results JSON
    json_path = output_dir / f"sub-{subject}_decoding_results.json"
