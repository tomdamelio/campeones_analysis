...
def run_pipeline() -> None:
    ...
    Steps:
        ...
        4. Run Leave-One-Video-Out CV with Scaler → PCA → Ridge.
    ...
    print("=" * 60)
    print(f"11 — Spectral Luminance Model (sub-{SUBJECT})")
    print("=" * 60)

    output_dir = RESULTS_PATH / "spectral"
    ...
    for train_entries, test_entries, test_video in folds:
        ...
        # Pipeline: StandardScaler → PCA → Ridge (GridSearchCV)
    ...
    def _build_and_evaluate(epoch_entries: list[dict]) -> float:
        ...
        for perm_train, perm_test, _ in perm_folds:
            ...
            pca_comp = min(PCA_COMPONENTS, perm_X_train.shape[0], perm_X_train.shape[1])
    ...
    plot_permutation_histogram(
        ...
        observed_r=perm_results["observed_r"],
        p_value=perm_results["p_value"],
        output_path=output_dir / f"sub-{SUBJECT}_spectral_model_permutation_hist.png",
    )
